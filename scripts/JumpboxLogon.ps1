...
if ($Env:installSQL2019 -eq "Yes") {
    Write-Header "$(Get-Date) - Installing SQL Server 2019 Containers"
...
if ($Env:installSQL2022 -eq "Yes") {
    Write-Header "$(Get-Date) - Installing SQL Server 2022 Containers"
    ...
    Write-Host "$(Get-Date) - Creating sql22 namespace"
    kubectl create namespace sql22

    Write-Host "$(Get-Date) - Creating Headless Services for SQL Pods"
    kubectl apply -f $Env:DeploymentDir\yaml\SQL2022\headless-services.yaml -n sql22

    Write-Host "$(Get-Date) - Setting sa password"
    kubectl create secret generic mssql22 --from-literal=MSSQL_SA_PASSWORD=$Env:adminPassword -n sql22

    Write-Host "$(Get-Date) - Applying kerberos configurations"
    kubectl apply -f $Env:DeploymentDir\yaml\SQL2022\krb5-conf.yaml -n sql22

    Write-Host "$(Get-Date) - Applying SQL Server configurations"
    kubectl apply -f $Env:DeploymentDir\yaml\SQL2022\mssql-conf.yaml -n sql22
...
"@
    ...
    $mssqlPodFile = "$Env:DeploymentDir\yaml\SQL2022\dxemssql.yaml"
    $mssqlPodScript | Out-File -FilePath $mssqlPodFile -force    
    kubectl apply -f $mssqlPodFile -n sql22    
...
spec:
  type: LoadBalancer
  loadBalancerIP: 10.$Env:vnetIpAddressRangeStr.5.0
  selector:
    #Assign load balancer to a specific pod
    statefulset.kubernetes.io/pod-name: mssql22-0
  ports:
  - name: sql
    protocol: TCP
    port: 1433
...
spec:
  type: LoadBalancer
  loadBalancerIP: 10.$Env:vnetIpAddressRangeStr.5.1
  selector:
    #Assign load balancer to a specific pod
    statefulset.kubernetes.io/pod-name: mssql22-1
  ports:
  - name: sql
    protocol: TCP
    port: 1433
...
spec:
  type: LoadBalancer
  loadBalancerIP: 10.$Env:vnetIpAddressRangeStr.5.2
  selector:
    #Assign load balancer to a specific pod
    statefulset.kubernetes.io/pod-name: mssql22-2
  ports:
  - name: sql
    protocol: TCP
    port: 1433
...
"@
    ...
    $mssqlPodServiceFile = "$Env:DeploymentDir\yaml\SQL2022\pod-service.yaml"
    $mssqlPodServiceScript | Out-File -FilePath $mssqlPodServiceFile -force    
    kubectl apply -f $mssqlPodServiceFile -n sql22
    ...
    $maxAttempts = 60
    while ((($podsDeployed -eq 0) -or ($servicesDeployed -eq 0)) -and ($attempts -le $maxAttempts)) {
        $pod_mssql22_0 = kubectl get pods -n sql22 mssql22-0 -o jsonpath="{.status.phase}"
        $pod_mssql22_1 = kubectl get pods -n sql22 mssql22-1 -o jsonpath="{.status.phase}"
        $pod_mssql22_2 = kubectl get pods -n sql22 mssql22-2 -o jsonpath="{.status.phase}"
        if (($pod_mssql22_0 -eq "Running") -and ($pod_mssql22_1 -eq "Running") -and ($pod_mssql22_2 -eq "Running")) {
            $podsDeployed = 1
        }
    
        $service_mssql22_0 = kubectl get services -n sql22 mssql22-0-lb -o jsonpath="{.spec.loadBalancerIP}"
        $service_mssql22_1 = kubectl get services -n sql22 mssql22-1-lb -o jsonpath="{.spec.loadBalancerIP}"
        $service_mssql22_2 = kubectl get services -n sql22 mssql22-2-lb -o jsonpath="{.spec.loadBalancerIP}"
        if (($service_mssql22_0 -eq "10.$Env:vnetIpAddressRangeStr.4.0") -and ($service_mssql22_1 -eq "10.$Env:vnetIpAddressRangeStr.4.1") -and ($service_mssql22_2 -eq "10.$Env:vnetIpAddressRangeStr.4.2")) {
            $servicesDeployed = 1
        }
    ...
    Write-Host "$(Get-Date) - Uploading keytab files to pods"
    $kubectlDeploymentDir = $Env:DeploymentDir -replace 'C:\\', '\..\'
    kubectl cp $kubectlDeploymentDir\keytab\SQL2022\mssql_mssql22-0.keytab mssql22-0:/var/opt/mssql/secrets/mssql.keytab -n sql22
    kubectl cp $kubectlDeploymentDir\keytab\SQL2022\mssql_mssql22-1.keytab mssql22-1:/var/opt/mssql/secrets/mssql.keytab -n sql22
    kubectl cp $kubectlDeploymentDir\keytab\SQL2022\mssql_mssql22-2.keytab mssql22-2:/var/opt/mssql/secrets/mssql.keytab -n sql22

    Write-Host "$(Get-Date) - Uploading logger.ini files to pods"
    kubectl cp "$kubectlDeploymentDir\yaml\SQL2022\logger.ini" mssql22-0:/var/opt/mssql/logger.ini -n sql22
    kubectl cp "$kubectlDeploymentDir\yaml\SQL2022\logger.ini" mssql22-1:/var/opt/mssql/logger.ini -n sql22
    kubectl cp "$kubectlDeploymentDir\yaml\SQL2022\logger.ini" mssql22-2:/var/opt/mssql/logger.ini -n sql22

    Write-Host "$(Get-Date) - Uploading TLS certificates to pods"
    kubectl cp "$kubectlDeploymentDir\certificates\SQL2022\mssql22-0.pem" mssql22-0:/var/opt/mssql/certs/mssql.pem -n sql22
    kubectl cp "$kubectlDeploymentDir\certificates\SQL2022\mssql22-0.key" mssql22-0:/var/opt/mssql/private/mssql.key -n sql22
    kubectl cp "$kubectlDeploymentDir\certificates\SQL2022\mssql22-1.pem" mssql22-1:/var/opt/mssql/certs/mssql.pem -n sql22
    kubectl cp "$kubectlDeploymentDir\certificates\SQL2022\mssql22-1.key" mssql22-1:/var/opt/mssql/private/mssql.key -n sql22
    kubectl cp "$kubectlDeploymentDir\certificates\SQL2022\mssql22-2.pem" mssql22-2:/var/opt/mssql/certs/mssql.pem -n sql22
    kubectl cp "$kubectlDeploymentDir\certificates\SQL2022\mssql22-2.key" mssql22-2:/var/opt/mssql/private/mssql.key -n sql22

    Write-Host "$(Get-Date) - Updating SQL Server Configurations"
    kubectl apply -f $Env:DeploymentDir\yaml\SQL2022\mssql-conf-encryption.yaml -n sql22
...
"@
    ...
    $sqlLoginFile = "$Env:DeploymentDir\scripts\CreateLogins.sql"
    $sqlLoginScript | Out-File -FilePath $sqlLoginFile -force
    SQLCMD -S "mssql22-0.$($Env:netbiosName.toLower()).$Env:domainSuffix" -U sa -P $Env:adminPassword -i $sqlLoginFile
    SQLCMD -S "mssql22-1.$($Env:netbiosName.toLower()).$Env:domainSuffix" -U sa -P $Env:adminPassword -i $sqlLoginFile
    SQLCMD -S "mssql22-2.$($Env:netbiosName.toLower()).$Env:domainSuffix" -U sa -P $Env:adminPassword -i $sqlLoginFile

    # Configure High Availability
    if ($Env:dH2iLicenseKey.length -eq 22) {
        Write-Header "$(Get-Date) - Configuring High Availability"
        ...
        $attempts = 1
        while (($licenseSuccess -eq 0) -and ($attempts -le $maxAttempts)) {
            $result = kubectl exec -n sql22 -c dxe mssql22-0 -- dxcli activate-server $Env:dH2iLicenseKey --accept-eula
            if ($result -eq "Result: License successfully set")	{
	    	    $licenseSuccess = 1
            }
            
            if (($licenseSuccess -eq 0) -and ($attempts -lt $maxAttempts)) {
                Write-Host "$(Get-Date) - Failed to obtain license for mssql22-0 - Attempt $attempts out of $maxAttempts"
                Start-Sleep -Seconds 10
            }
            $attempts += 1
        }
        if ($licenseSuccess -eq 0) {
            Write-Host "$(Get-Date) - Failed to obtain license for mssql22-0 - $result"
        }

        $licenseSuccess = 0
        $attempts = 1
        while (($licenseSuccess -eq 0) -and ($attempts -le $maxAttempts)) {
            $result = kubectl exec -n sql22 -c dxe mssql22-1 -- dxcli activate-server $Env:dH2iLicenseKey --accept-eula
            if ($result -eq "Result: License successfully set")	{
	    	    $licenseSuccess = 1
            }
            
            if (($licenseSuccess -eq 0) -and ($attempts -lt $maxAttempts)) {
                Write-Host "$(Get-Date) - Failed to obtain license for mssql22-1 - Attempt $attempts out of $maxAttempts"
                Start-Sleep -Seconds 10
            }
            $attempts += 1
        }
        if ($licenseSuccess -eq 0) {
            Write-Host "$(Get-Date) - Failed to obtain license for mssql22-1 - $result"
        }
        
        $licenseSuccess = 0
        $attempts = 1
        while (($licenseSuccess -eq 0) -and ($attempts -le $maxAttempts)) {
            $result = kubectl exec -n sql22 -c dxe mssql22-2 -- dxcli activate-server $Env:dH2iLicenseKey --accept-eula
            if ($result -eq "Result: License successfully set")	{
	    	    $licenseSuccess = 1
            }
            
            if (($licenseSuccess -eq 0) -and ($attempts -lt $maxAttempts)) {
                Write-Host "$(Get-Date) - Failed to obtain license for mssql22-2 - Attempt $attempts out of $maxAttempts"
                Start-Sleep -Seconds 10
            }
            $attempts += 1
        }
        if ($licenseSuccess -eq 0) {
            Write-Host "$(Get-Date) - Failed to obtain license for mssql22-2 - $result"
        }

        Write-Host "$(Get-Date) - Creating HA Cluster on mssql22-0"
        kubectl exec -n sql22 -c dxe mssql22-0 -- dxcli cluster-add-vhost mssql22-agl1 *127.0.0.1 mssql22-0

        Write-Host "$(Get-Date) - Getting encrypted password for sa"
        $saSecurePassword = kubectl exec -n sql22 -c dxe mssql22-0 -- dxcli encrypt-text $Env:adminPassword

        Write-Host "$(Get-Date) - Creating Availability Group on mssql22-0"
        kubectl exec -n sql22 -c dxe mssql22-0 -- dxcli add-ags mssql22-agl1 mssql22-ag1 "mssql22-0|mssqlserver|sa|$saSecurePassword|5022|synchronous_commit|0" "CONTAINED"
        Start-Sleep -Seconds 30

        Write-Host "$(Get-Date) - Setting the cluster passkey using admin password"
        kubectl exec -n sql22 -c dxe mssql22-0 -- dxcli cluster-set-secret-ex $Env:adminPassword

        Write-Host "$(Get-Date) - Enabling vhost lookup in DxEnterprise's global settings"
        kubectl exec -n sql22 -c dxe mssql22-0 -- dxcli set-globalsetting membername.lookup true

        Write-Host "$(Get-Date) - Joining mssql22-1 to cluster"
        kubectl exec -n sql22 -c dxe mssql22-1 -- dxcli join-cluster-ex mssql22-0 $Env:adminPassword

        Write-Host "$(Get-Date) - Joining mssql22-1 to the Availability Group"
        kubectl exec -n sql22 -c dxe mssql22-1 -- dxcli add-ags-node mssql22-agl1 mssql22-ag1 "mssql22-1|mssqlserver|sa|$saSecurePassword|5022|synchronous_commit|0"
        Start-Sleep -Seconds 30

        Write-Host "$(Get-Date) - Joining mssql22-2 to cluster"
        kubectl exec -n sql22 -c dxe mssql22-2 -- dxcli join-cluster-ex mssql22-0 $Env:adminPassword

        Write-Host "$(Get-Date) - Joining mssql22-2 to the Availability Group"
        kubectl exec -n sql22 -c dxe mssql22-2 -- dxcli add-ags-node mssql22-agl1 mssql22-ag1 "mssql22-2|mssqlserver|sa|$saSecurePassword|5022|synchronous_commit|0"
        Start-Sleep -Seconds 30
...
kind: Service
metadata:
  name: mssql22-cluster-lb
  annotations:
    service.beta.kubernetes.io/azure-load-balancer-internal: "true"
spec:
  type: LoadBalancer
  loadBalancerIP: 10.$Env:vnetIpAddressRangeStr.5.3
  selector:
    app: mssql22
  ports:
  - name: sql
...
"@
        ...
        $mssqlListenerServiceFile = "$Env:DeploymentDir\yaml\SQL2022\service.yaml"
        $mssqlListenerServiceScript | Out-File -FilePath $mssqlListenerServiceFile -force
        kubectl apply -f $mssqlListenerServiceFile -n sql22
        ...
        $maxAttempts = 60
        while (($listenerDeployed -eq 0) -and ($attempts -le $maxAttempts)) {
            $service_mssql22_agl1 = kubectl get services -n sql22 mssql22-cluster-lb -o jsonpath="{.spec.loadBalancerIP}"
            if ($service_mssql22_agl1 -eq "10.$Env:vnetIpAddressRangeStr.4.3") {
                $listenerDeployed = 1
            }
        ...
        Write-Host "$(Get-Date) - Copying backup file to mssql22-0"
        kubectl cp $kubectlDeploymentDir\backups\AdventureWorks2019.bak mssql22-0:/var/opt/mssql/backup/AdventureWorks2019.bak -n sql22

        Write-Host "$(Get-Date) - Restoring database backup to mssql22-0 and configuring for High Availability"
$sqlRestoreScript = @"
RESTORE DATABASE AdventureWorks2019
FROM DISK = N'/var/opt/mssql/backup/AdventureWorks2019.bak'
WITH
...
"@
        ...
        $sqlRestoreFile = "$Env:DeploymentDir\scripts\RestoreDatabase.sql"
        $sqlRestoreScript | Out-File -FilePath $sqlRestoreFile -force
        SQLCMD -S "mssql22-0.$($Env:netbiosName.toLower()).$Env:domainSuffix" -U sa -P $Env:adminPassword -i $sqlRestoreFile
